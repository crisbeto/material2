...
import {RxChain} from '../core/util/rx-chain';
import {map, _do, _catch, _finally, share} from '../core/util/rx-operators';
...
export class MdIconRegistry {
  ...
  getSvgIconFromUrl(safeUrl: SafeResourceUrl): Observable<SVGElement> {
    ...
    if (cachedIcon) {
      return of.call(Observable, cloneSvg(cachedIcon));
    }

    return RxChain.from(this._loadSvgIconFromConfig(new SvgIconConfig(url)))
      .call(_do, svg => this._cachedIconsByUrl.set(url!, svg))
  ...
  getNamedSvgIcon(name: string, namespace = ''): Observable<SVGElement> {
    ...
    return _throw.call(Observable, getMdIconNameNotFoundError(key));
  }
  ...
  private _getSvgFromConfig(config: SvgIconConfig): Observable<SVGElement> {
    if (config.svgElement) {
      ...
      return of.call(Observable, cloneSvg(config.svgElement));
    } else {
      // Fetch the icon from the config's URL, cache it, and return a copy.
      return RxChain.from(this._loadSvgIconFromConfig(config))
          .call(_do, svg => config.svgElement = svg)
          .call(map, svg => cloneSvg(svg))
          .result();
    }
  }
  ...
  private _getSvgFromIconSetConfigs(name: string, iconSetConfigs: SvgIconConfig[]):
    ...
    if (namedIcon) {
      ...
      return of.call(Observable, namedIcon);
    }
  ...
  private _fetchUrl(safeUrl: SafeResourceUrl): Observable<string> {
    ...
    const req = RxChain.from(this._http.get(url))
      ...
      .call(_finally, () => this._inProgressUrlFetches.delete(url))
      .call(share)
      .result();
